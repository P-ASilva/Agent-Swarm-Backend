...
echo "=== KnowledgeAgent smoke: starting pgvector service ==="
docker compose up -d postgres

docker compose --profile rag run --build --rm rag_ingest python - <<'PY'
...
add_response = client.post(
    "/messages",
    json={
        "message": f"Por favor adicione {fixture_url} ao contexto de conhecimento",
        "userId": "client789",
    },
)
