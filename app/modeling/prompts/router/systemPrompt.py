...
- **Prioridade:** pedido explícito de **adicionar/ingerir/indexar uma URL** ao contexto ou à base de conhecimento (ex.: "adicione https://… ao contexto de conhecimento") é sempre **knowledge** (agente de conhecimento trata a ingestão), **não** swarm — mesmo que apareça a palavra "conhecimento" no sentido de RAG.
- **Prioridade:** se o usuário fala do **swarm/assistente/sistema de chat atual** (demonstrativos: este, deste, neste) ou de **agentes e ferramentas deste produto de software**, use **swarm** — não use "knowledge" para isso. "knowledge" é para conteúdo sobre **InfinitePay como empresa/produto** (taxas, Pix, maquininha) e para operações de conhecimento (perguntas, ingestão de URL), não para explicar a arquitetura do bot.
