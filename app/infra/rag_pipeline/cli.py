...
from app.infra.rag_pipeline.embed import buildEmbeddingProviderFromEnv
...
def _buildParser() -> argparse.ArgumentParser:
    ...
    defaultManifestPath = os.getenv("RAG_SEED_URLS_PATH", "app/infra/rag_pipeline/seedUrls.json")
    defaultCrawlStamp = datetime.now(UTC).strftime("%Y%m%d")
    ...
    for commandName, helpText in (
        ...
        _attachIngestArgs(sub, defaultManifestPath=defaultManifestPath, defaultCrawlStamp=defaultCrawlStamp)
    ...
    addUrl.add_argument("--crawl-version", default=defaultCrawlStamp)
...
def _attachIngestArgs(
    sub: argparse.ArgumentParser,
    *,
    defaultManifestPath: str,
    defaultCrawlStamp: str,
) -> None:
    ...
    sub.add_argument("--crawl-version", default=defaultCrawlStamp)
...
def _runIngestion(args: argparse.Namespace, *, runType: str) -> int:
    ...
    effectiveCrawl = args.crawl_version or datetime.now(UTC).strftime("%Y%m%d")
    manifestPath = args.manifest_path
    if isinstance(manifestPath, str):
        manifestPath = manifestPath.strip()
